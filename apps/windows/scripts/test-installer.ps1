...
$installDir = Join-Path $env:LOCALAPPDATA 'Notch Test Zoë'
$settings = Join-Path $env:USERPROFILE '.claude/settings.json'
if (Test-Path $settings) { throw 'Refusing to overwrite existing Claude settings.' }
$startupName = if ($env:NOTCH_CHANNEL -eq 'stable') { 'Notch' } else { 'Notch Preview' }
$runKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
if (Get-ItemProperty $runKey -Name $startupName -ErrorAction SilentlyContinue) {
...
$app = Join-Path $installDir 'notch.exe'
$helper = Join-Path $installDir 'notch-hook.exe'
...
if (!$before.Contains('notch-hook.exe')) { throw 'Installed helper was not wired.' }
Run-Checked $app 'autostart on'
$startup = (Get-ItemProperty $runKey).$startupName
...
if ((Get-ItemProperty $runKey).$startupName -ne $startup) { throw 'Update changed startup.' }
...
if ((Get-Content $settings -Raw).Contains('notch-hook.exe')) { throw 'Uninstall left broken hooks.' }
if (Get-ItemProperty $runKey -Name $startupName -ErrorAction SilentlyContinue) {
