...
pub fn run() -> String {
    ...
    match std::net::TcpListener::bind(("127.0.0.1", cfg.port)) {
        Ok(_) => o += "port: free — no Notch instance is running\n",
    ...
    if let Some(dir) = dirs::config_dir() {
        let p = dir.join("notch").join("watch.log");
