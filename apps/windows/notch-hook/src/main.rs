//! notch-hook: the minimal client Claude Code's hooks call.
...
#[path = "../../identity.rs"]
mod identity;
const DEFAULT_PORT: u16 = identity::PORT;
...
/// Pulls "port": N out of %APPDATA%\notch\config.json (hand-rolled scan, no dependency)
fn read_port() -> u16 {
    let path = match std::env::var("APPDATA") {
        Ok(a) => format!("{a}\\{}\\config.json", identity::NAMESPACE),
