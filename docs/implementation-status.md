...
- Windows Command Code no longer requires an organization: like the Mac app, a `whoami` reply without one omits `orgId` from billing queries. Awaiting native CI; live account validation remains pending.
