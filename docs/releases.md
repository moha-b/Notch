...
Only after both signed packages are public are the Sparkle and Tauri feeds generated and deployed together as one Pages artifact. A signing, packaging, publication or deployment failure leaves the previous deployed feeds in place. If assets became public but feed preparation or Pages failed, rerun the failed jobs in the original workflow while its signed build artifacts remain available. Publication retries verify the existing public manifest, source tag, package checksums, signatures and notes before regenerating feeds; they never replace assets or resume a draft. Retries reject releases older than an already published version. If a signed package itself is wrong, qualify and publish a higher version.
